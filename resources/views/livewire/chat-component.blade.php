<div class="flex flex-col bg-gray-900 text-gray-200 p-4 rounded-lg">
    ...
    <div class="overflow-y-auto h-48 mb-3 space-y-2" id="messages">
        @foreach ($messages as $message)
            <div class="flex {{ $message['user'] === 'You' ? 'justify-end' : 'justify-start' }}">
                <div class="max-w-md">
                    <div
                        class="{{ $message['user'] === 'You' ? 'bg-indigo-600' : 'bg-gray-700' }} shadow rounded-xl p-2">
                        <p class="text-sm whitespace-pre-wrap">
                            {{ $message['text'] }}
                        </p>
                    </div>
                    <span
                        class="block text-xs mt-1 text-gray-400 {{ $message['user'] === 'You' ? 'text-right' : 'text-left' }}">
                        {{ $message['user'] }}
                    </span>
                </div>
            </div>
    ...
    <div class="border-t border-gray-700 pt-2">
        <form wire:submit.prevent="send" class="flex space-x-2">
            <input type="text" wire:model.defer="input" placeholder="Ask something..."
        ...
        <div class="space-y-2">
            @foreach ($todos as $index => $todo)
                <div class="flex items-center gap-2">
                    <input type="checkbox" wire:click="toggleTodo({{ $todo['id'] }})" @checked($todo['completed'])
